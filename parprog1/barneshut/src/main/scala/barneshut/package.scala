...
package object barneshut {
  ...
  case class Empty(centerX: Float, centerY: Float, size: Float) extends Quad {
    ...
    def insert(b: Body): Quad = Leaf(centerX, centerY, size, Vector(b))
  }
  ...
  case class Fork(
    ...
    val size: Float = (ne.centerX - nw.centerX) * 2
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        def loop(bodies: Seq[Body], fork: Fork): Fork = bodies match {
          case Nil => fork
          case x +: xs => loop(xs, fork.insert(x))
        }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      def relativePosition(value: Float, min: Float, max: Float): Float = value match {
        case x if x < min => 0
        case x if x > max => max - min
        case x => x - min
      }

      val sectorX = (relativePosition(b.x, boundaries.minX, boundaries.maxX) / sectorSize).toInt
      val sectorY = (relativePosition(b.y, boundaries.minY, boundaries.maxY) / sectorSize).toInt
      apply(sectorX, sectorY) += b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      for {
        i <- result.matrix.indices
      } yield result.matrix(i) = this.matrix(i) combine that.matrix(i)
      result
    }
