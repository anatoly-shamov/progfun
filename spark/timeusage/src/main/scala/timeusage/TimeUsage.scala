...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(
      StructField(columnNames.head, StringType, nullable = false, Metadata.empty) +:
        columnNames.tail.map(StructField(_, DoubleType, nullable = false, Metadata.empty))
    )
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row(line.head +: line.tail.map(_.toDouble): _*)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val workingActivities = ((e: String) =>
      e.startsWith("t05")
        || e.startsWith("t1805")
      )

    val leisure = ((e: String) =>
      (e.startsWith("t02")
        || e.startsWith("t04")
        || e.startsWith("t06")
        || e.startsWith("t07")
    ...
    (
      columnNames.filter(primaryNeeds).map(col),
      columnNames.filter(workingActivities).map(col),
      columnNames.filter(leisure).map(col)
    )
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column =
      when(col("teage") >= 15 && col("teage") <= 22, "young")
        .otherwise(when(col("teage") >= 23 && col("teage") <= 55, "active")
          .otherwise("elder"))
        .as("age")
    ...
    def sumColumns(columns: List[Column]): Column = columns.tail.foldLeft(columns.head / 60)(_ + _ / 60)
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(e =>
      (e.working, e.sex, e.age)
    ).agg(
      round(typed.avg((row: TimeUsageRow) => row.primaryNeeds), 1).as[Double],
      round(typed.avg((row: TimeUsageRow) => row.work), 1).as[Double],
      round(typed.avg((row: TimeUsageRow) => row.other), 1).as[Double]
    ).map {
      case ((k1, k2, k3), v1, v2, v3) => TimeUsageRow(k1, k2, k3, v1, v2, v3)
